...
If the user's real goal is "is this environment ready to deploy to," that's a read-only health check (list targets for the environment/tenant, check health status) — a candidate for step-level delegation if it returns a long list, per `octopus_workflow_monitoring_diagnostics`'s guidance. It also overlaps with `octopus_workflow_release_deployment`'s Step 2 readiness check — point there if the user is mid-deployment.
