...
If the user is setting up a brand-new project end to end (create project → add deployment process steps → deploy), point them to `octopus-release-deployment` for the deploy portion once the process is in place, rather than duplicating that flow here. If the project is (or should be) connected to a git repository for its deployment process/settings, that's `octopus-config-as-code`'s job — deployment settings above assume the plain database-backed form; the git-backed form works differently and is covered there.
