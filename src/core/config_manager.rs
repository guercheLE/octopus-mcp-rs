...
/// (`./octopus-mcp.config.yml`) -> home file
/// (`~/.octopus-mcp/config.yml`) -> system file
/// (`/etc/octopus-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/octopus-mcp/config.yml")),
...
mod tests {
    ...
    fn base_flags() -> Map<String, Value> {
        json!({ "url": "https://api.example.com", "auth_method": "apiKey" })
            .as_object()
            .unwrap()
            .clone()
    }
