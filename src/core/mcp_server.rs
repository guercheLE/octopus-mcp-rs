...
/// `"type": "object"`, even for zero-parameter operations — so a missing
/// `arguments` field must default to a real empty object, not `Value::Null`
/// (which always fails that schema's validation), for the `call` tool to
/// work at all against a parameterless operation.
...
pub struct CallArgs {
    ...
    /// omitted (not `null` — every operation's generated schema declares
    /// `"type": "object"`, so `null` always fails validation).
...
impl McpifyServer {
    ...
    #[tool(
        description = "Semantic search for Octopus Server API operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific Octopus Server API operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live Octopus Server API API operation, and validate the response."
    )]
    async fn call(
        &self,
