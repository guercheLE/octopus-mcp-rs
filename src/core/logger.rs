...
const LOG_LEVEL_ENV_VAR: &str = "OCTOPUS_MCP_LOG_LEVEL";
const TRANSPORT_ENV_VAR: &str = "OCTOPUS_MCP_TRANSPORT";
...
/// `OCTOPUS_MCP_LOG_LEVEL`, defaulting to `"info"` when unset.
...
pub fn init_logging(otel_layer: Option<BoxedLayer>) {
    ...
    // Logs go to stderr in stdio mode. In HTTP mode stdout carries no
    // protocol traffic, so logs go there instead — the conventional
    // destination for a long-running network service.
