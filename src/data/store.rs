...
const VERSION_STORE_BYTES: &[(&str, &[u8])] =
    &[("2023.4.82.90", include_bytes!("../../mcp_store.db.zst"))];
// mcpify:versions:end
...
/// it, except `octopus-mcp-populate-embeddings` (a separate `[[bin]]`), which uses
...
/// `octopus-mcp-populate-embeddings` run, a deployment replacing the file) can update
